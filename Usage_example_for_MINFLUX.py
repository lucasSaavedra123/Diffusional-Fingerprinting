...
if __name__ == '__main__':
    for DATASET_PATH in glob.glob('*.csv'):
        print(DATASET_PATH)
        DATASETS_LABELS = DATASET_PATH.split('.')[0].split('_')
        ...
        m = np.round(m.astype('float') / m.sum(axis=1)[:, np.newaxis], 2)
        ...
        fig.tight_layout()
        fig.savefig(f"{FILE_LABEL}_Confusion_matrix")
        ...
        plt.tight_layout()
        plt.savefig(f"{FILE_LABEL}_Feature_ranking")
